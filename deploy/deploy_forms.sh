...
mkdir -p /opt/cw/"$APP_NAME"/"$CODE"
cd /opt/cw/"$APP_NAME"/"$CODE"
tar zcf /opt/cw/backups/"$APP_NAME"-"$CODE".tar.gz .
echo "Backup Current Client Complete."

# Unzip the Folder of the App into the Temp Folder
mkdir -p /opt/cw/"$APP_NAME"/"$CODE".build
cd /opt/cw/"$APP_NAME"/"$CODE".build
...
cd /opt/cw/"$APP_NAME"
rm -r /opt/cw/"$APP_NAME"/"$CODE"
mv /opt/cw/"$APP_NAME"/"$CODE".build /opt/cw/"$APP_NAME"/"$CODE"
